...
if (isset($_REQUEST['details_id'])){
	...
	<div class="inside">
	<?php
	$q = $wpdb->prepare("SELECT * FROM $table_name WHERE id=%d ", $_REQUEST['details_id']);
	$data = $wpdb->get_row($q);
...
$q = $wpdb->prepare("SELECT * FROM $table_name ORDER BY paydate DESC LIMIT %d OFFSET %d", $limit, $offset);
